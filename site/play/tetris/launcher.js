...
const gameId = "tetris";
const expectedSha1 = "74591cc9501af93873f9a5d3eb12da12c0723bbc";
const expectedSize = 32768;
...
async function findGame() {
  ...
  if (!game)
    throw new Error("Tetris is missing from the catalog");
  return game;
}
...
async function launch(file) {
  ...
  if (!game.browser.ready || !game.browser.module) {
    report("ROM VERIFIED. THE TETRIS BROWSER ADAPTER IS THE NEXT BUILD STEP.", "good");
    return;
  }

  report("STARTING NATIVE RUNTIME…", "good");
  ...
  try {
    activeRuntime = await runtime.start({
      ...
      onStopped: () => {
        ...
        enableAudio.hidden = true;
        report("THE NATIVE RUNTIME STOPPED. SUPPLY THE CARTRIDGE TO START AGAIN.");
      },
    });
